...
.DESCRIPTION
    ...
    By default the script:
      ...
      2. tests the production exact-tile strip decoder across multiple worker counts;
...
function Parse-TraceFile {
    ...
    foreach ($line in $lines) {
        if ($line -match '\tevent=info\tmessage=parallel strip path requested=([0-9]+), workers=([0-9]+),') {
            $partition = 'strip'
            $actualWorkers = [int]$matches[2]
            $route = 'parallel-strip'
            break
        }
    ...
    foreach ($line in $lines) {
        if ($line -match '\tevent=info\tmessage=stages mode=strip, setup-mean=([0-9.]+), setup-max=([0-9.]+), copy-mean=([0-9.]+), copy-max=([0-9.]+), convert-mean=([0-9.]+), convert-max=([0-9.]+)') {
...
try {
    ...
    for ($repeat = 1; $repeat -le $Repeats; $repeat++) {
        ...
        foreach ($worker in $workerOrder) {
            ...
            Write-Host ("Run {0}/{1}  requested={2,2} ..." -f $repeat, $Repeats, $worker) -NoNewline
            ...
            try {
                ...
                Write-Host (" {0,8:N1} ms  actual={1,2}{2}" -f $result.DecodeMs, $result.ActualWorkers, $stageText) -ForegroundColor Green
            }
            catch {
...
$summary |
    ...
    Format-Table Worker, ActualWorkers, Runs, MedianMs, SetupMaxMedianMs, CopyMaxMedianMs, ConvertMaxMedianMs, RelativeToBest, Route -AutoSize

if ($validSummary.Count -gt 0) {
    $winner = $validSummary | Sort-Object MedianMs | Select-Object -First 1
    Write-Host ("Best median: FASTJXR_WORKERS={0} (actual={1})  ({2:N3} ms)" -f $winner.Worker, $winner.ActualWorkers, $winner.MedianMs) -ForegroundColor Green
}
